...
-- Delta load notebook bfx_awlt_ProductModel
...
-- COMMAND ----------
DROP TABLE IF EXISTS `TMP_AdventureWorksLT2012_SalesLT_ProductModel_CURRENT`;
CREATE TABLE `TMP_AdventureWorksLT2012_SalesLT_ProductModel_CURRENT` AS 
...
(
    ...
    FROM	`${_bfx_ods}`.`awlt`.`ProductModel` CUR
    INNER JOIN `TMP_AdventureWorksLT2012_SalesLT_ProductModel` TMP
...
(
	...
	LEFT OUTER JOIN `TMP_AdventureWorksLT2012_SalesLT_ProductModel_CURRENT` AS TGT
...
-- COMMAND ----------
DROP TABLE IF EXISTS `TMP_AdventureWorksLT2012_SalesLT_ProductModel_EXISTS`;
CREATE TABLE `TMP_AdventureWorksLT2012_SalesLT_ProductModel_EXISTS` AS 
SELECT	 STG.`ProductModelID`
		,STG.`FlexRowHash`
		,STG.`FlexRowEffectiveFromDate`
		,STG.`FlexRowChangeType`
FROM	`TMP_AdventureWorksLT2012_SalesLT_ProductModel_DELTA` STG
INNER JOIN
(
	SELECT	 SRC.`ProductModelID`
...
INNER JOIN `TMP_AdventureWorksLT2012_SalesLT_ProductModel_CURRENT` CUR
...
SELECT   SRC.`ProductModelID`
		...
		,SRC.`FlexRowHash`
FROM	`TMP_AdventureWorksLT2012_SalesLT_ProductModel_DELTA` SRC
LEFT OUTER JOIN `TMP_AdventureWorksLT2012_SalesLT_ProductModel_CURRENT` TGT
    ON  SRC.`ProductModelID` = TGT.`ProductModelID`
LEFT OUTER JOIN `TMP_AdventureWorksLT2012_SalesLT_ProductModel_EXISTS` PSA
