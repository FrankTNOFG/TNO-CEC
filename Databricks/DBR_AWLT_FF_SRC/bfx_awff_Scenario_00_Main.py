...
if (spark.sql("SELECT COUNT(*) FROM `" + _bfx_ods + "`.`awff`.`Scenario`").collect()[0][0] == 0):
    dbutils.notebook.run("./bfx_awff_Scenario_01_Full", 0, {"row_audit_id": row_audit_id, "location_path": location_path, "_bfx_stg": _bfx_stg, "_bfx_ods": _bfx_ods})
else:
    dbutils.notebook.run("./bfx_awff_Scenario_02_Delta", 0, {"row_audit_id": row_audit_id, "location_path": location_path, "_bfx_stg": _bfx_stg, "_bfx_ods": _bfx_ods})